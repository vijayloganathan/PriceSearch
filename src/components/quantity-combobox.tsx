...
export default function QuantityCombobox({ quantityTypes = [], value, onChange }: QuantityComboboxProps) {
  const [open, setOpen] = React.useState(false);
  const isMobile = useIsMobile();
  
  const options = React.useMemo(() => {
    const existingTypes = [...new Set((quantityTypes || []).map(pt => pt.name).filter(Boolean))];
...
function TypeList({
  ...
  return (
    <Command>
      <CommandInput placeholder="Search quantity..." />
      <CommandList>
        <CommandEmpty>No quantity found.</CommandEmpty>
        <CommandGroup>
